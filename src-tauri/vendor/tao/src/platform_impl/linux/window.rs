...
impl Window {
  pub(crate) fn new<T>(
    ...
    // dsh-desktop patch: do NOT install tao's GtkHeaderBar on Wayland at all.
    // tao used to force a CSD titlebar on every Wayland window, which showed a
    // GNOME-style header on tiling compositors like driftwm (their SSD mode
    // already provides the chrome). Without the forced header, GTK3 negotiates
    // xdg-decoration with the compositor, so the environment decides: SSD on
    // driftwm (no titlebar), a normal titlebar on GNOME/KDE. See
